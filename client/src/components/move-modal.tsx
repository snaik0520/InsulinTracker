...
export function MoveModal({ open, onOpenChange, medication }: MoveModalProps) {
  ...
  const incrementQuantity = () => {
    if (!medication) return;
    const current = moveQuantity === "" ? 0 : (typeof moveQuantity === "number" ? moveQuantity : parseInt(moveQuantity, 10));
  ...
  const qtyNumber = typeof moveQuantity === "number" ? moveQuantity : (moveQuantity ? parseInt(moveQuantity, 10) : 0);
