...
import { insertMedicationSchema, insertTransactionSchema, moveTransactionSchema, TransactionType } from "@shared/schema";
import { z } from "zod";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/medications/move", async (req, res) => {
    try {
      ...
      // Verify the medication is currently at the fromLocation
      if (medication.location !== fromLocation) {
        return res.status(400).json({ 
          error: `Medication is not at ${fromLocation}. Current location: ${medication.location}` 
        });
      }
      ...
      if (medication.quantity === quantity) {
        ...
        // Create a new medication record for the moved portion
      ...
      await storage.createTransaction({
        ...
        quantity: quantity,
        notes: notes || `Moved from ${fromLocation} to ${toLocation}`,
      ...
      res.json({
        success: true,
        message: `Successfully moved ${quantity} units from ${fromLocation} to ${toLocation}`,
        updatedMedication
      });
    } catch (error) {
      if (error instanceof z.ZodError) {
